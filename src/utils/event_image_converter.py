...
class EventImageConverter(object):
    ...
    Originally from https://github.com/tub-rip/event_based_optical_flow.
    ...
    def create_iwe(
        ...
        if isinstance(events, torch.Tensor):
            iwes = self.create_image_from_events_tensor(events, method, sigma=sigma, weight=weight)
    ...
    def create_eventmask(self, events: NUMPY_TORCH, threshold: float=0.0) -> NUMPY_TORCH:
        """Create mask image where at least one event exists.

        Args:
            events (NUMPY_TORCH): [(b,) n_events, 4]
            threshold (float): threshold of the event mask.

        Returns:
            NUMPY_TORCH: [(b,) 1, H, W]
        """
        if isinstance(events, torch.Tensor):
            return (threshold < self.create_image_from_events_tensor(events, sigma=0))[..., None, :, :]
        elif isinstance(events, np.ndarray):
            return (threshold < self.create_image_from_events_numpy(events, sigma=0))[..., None, :, :]
        raise RuntimeError
    ...
    def create_image_from_events_tensor(
        ...
        if method == "count":
            ...
            if isinstance(weight, torch.Tensor):
                pos_image = self.bilinear_vote_tensor(events[pos_flag], weight=weight[pos_flag])
                neg_image = self.bilinear_vote_tensor(events[~pos_flag], weight=weight[~pos_flag])
            else:
                pos_image = self.bilinear_vote_tensor(events[pos_flag], weight=weight)
                neg_image = self.bilinear_vote_tensor(events[~pos_flag], weight=weight)
