...
class VoxelGrid(EventRepresentation):
    def __init__(self, input_size: tuple, norm_type: str, quantile):
        assert len(input_size) == 3
        self.voxel_grid = torch.zeros((input_size), dtype=torch.float, requires_grad=False)
        self.nb_channels = input_size[0]
        self.norm_type = norm_type
        assert self.norm_type in ['mean_std', 'max', None]
        self.quantile = quantile
        assert 0 <= self.quantile < 0.15
    ...
    def convert(self, events):
        ...
        with torch.no_grad():
            ...
            if self.quantile > 0:
                threshold_value = torch.quantile(voxel_grid.abs().view(-1), 1 - self.quantile)
                voxel_grid = torch.where(voxel_grid.abs() > threshold_value,
                                         voxel_grid.sign() * threshold_value,
                                         voxel_grid)
            ...
            if self.norm_type == 'mean_std':
                ...
                max_value = voxel_grid.abs().max()
                if max_value > 0:
                    voxel_grid = voxel_grid / max_value
